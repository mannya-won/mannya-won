## Hi there 👋 I'm Mannya

🎓 4th Year B.Tech Student in **Electronics and Communication Engineering**

💡 Interested in **VLSI Design**, **Digital Systems** , **FPGA** & **HDL programming**

⚙️ Comfortable with **Arduino IDE**, **basic MATLAB**, and **Python**


## 🔧 Skills
- Arduino (basic projects, sensor interfacing)  
- Python (beginner level)  
- MATLAB (basic simulations)

## 📂 Projects
- **Wet, Metal and Dry Waste Segregation System** – [Click here](https://github.com/mannya-won/WetMetalDry_wasteSegregation)  
- **Arduino-based stopwatch with lap time functionality** : [Click here](https://github.com/mannya-won/Stopwatch-Project)

## 📫 Connect with Me
- [LinkedIn](www.linkedin.com/in/mannyakinframe)  
- [Email](mannyak09762@gmail.com)   
